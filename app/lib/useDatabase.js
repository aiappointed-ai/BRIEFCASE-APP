...
if (online) {
if (data.id) {
const { data: u, error } = await supabase.from('coaches').update(data).eq('id', data.id).select().single();
if (!error && u) setCoaches(p => p.map(c => c.id === u.id ? u : c));
} else {
const { id, ...rest } = data;
const { data: c, error } = await supabase.from('coaches').insert({ ...rest, created_by: userId }).select().single();
if (!error && c) setCoaches(p => [...p, c]);
}
} else {
...
const { data: u, error } = await supabase.from('players').update(data).eq('id', data.id).select().single();
if (!error && u) setPlayers(p => p.map(x => x.id === u.id ? u : x));
} else {
const { id, ...rest } = data;
const { data: c, error } = await supabase.from('players').insert({ ...rest, scouted_by: userId }).select().single();
if (!error && c) setPlayers(p => [c, ...p]);
}
} else {
